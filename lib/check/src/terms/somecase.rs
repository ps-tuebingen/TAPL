    ...
    Self: Into<Lang::Term>,
{
    ...
    fn check(&self, env: Environment<Lang>) -> Result<Derivation<Self::Lang>, CheckError> {
        ...
        let option = bound_norm.clone().into_optional().ok_or_else(|| {
            TypeMismatch::new(bound_norm.to_string(), "Option Type".to_string(), self.span)
        })?;
        ...
        if some_norm != none_norm {
            return Err(
                TypeMismatch::new(some_norm.to_string(), none_norm.to_string(), self.span).into(),
            );
        }
