    ...
    Self: Into<Lang::Term>,
{
    ...
    fn check(&self, env: Environment<Lang>) -> Result<Derivation<Self::Lang>, CheckError> {
        ...
        let conc = TypingConclusion::new(env, self.clone(), Optional::new(ty_norm, self.span));
        let deriv = TypingDerivation::something(conc, premises);
        Ok(deriv.into())
    }

    fn rules() -> HashSet<DerivationRule> {
        let term = vec![Keyword::Something.into(), Symbol::paren(Symbol::Term)];
        let ty_res = vec![Keyword::Optional.into(), Symbol::sqbrack(Symbol::Type)];
