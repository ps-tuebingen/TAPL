...
impl<Lang> FreeVars for Cons<Lang>
where
    Lang: Language,
{
    fn free_vars(&self, vars: &mut HashSet<Var>) {
        self.head.free_vars(vars);
        self.tail.free_vars(vars);
    }
}

impl<Lang> FreeTypeVars for Cons<Lang>
where
    Lang: Language,
{
    fn free_type_vars(&self, vars: &mut HashSet<TypeVar>) {
        self.head.free_type_vars(vars);
        self.tail.free_type_vars(vars);
