...
impl<Lang> FreeVars for Pair<Lang>
where
    Lang: Language,
{
    fn free_vars(&self, vars: &mut HashSet<Var>) {
        self.fst.free_vars(vars);
        self.snd.free_vars(vars);
    }
}

impl<Lang> FreeTypeVars for Pair<Lang>
where
    Lang: Language,
{
    fn free_type_vars(&self, vars: &mut HashSet<TypeVar>) {
        self.fst.free_type_vars(vars);
        self.snd.free_type_vars(vars);
    }
}
