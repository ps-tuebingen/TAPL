...
use syntax::{
    ...
    span::Spanned,
    subst::SubstType,
    types::{ExistsBounded, TypeGroup, TypeVariable},
};

impl<Lang> SolveConstraint for ExistsBounded<Lang>
where
    Lang: Language,
    ...
    TypeVariable<Lang>: Into<Lang::Type>,
{
    ...
    fn solve_equality(
        ...
        let rhs_exists = rhs.into_exists_bounded().ok_or(err)?;
    ...
    fn solve_subtyping(
        ...
        let sup_exists = sup.into_exists_bounded().ok_or(err)?;
        ...
        state.add_constraint(SubtypeConstraint::new(
            ...
            Rc::unwrap_or_clone(sup_exists.sup_ty),
        ));
