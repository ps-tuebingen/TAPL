...
use syntax::{
    ...
    span::Spanned,
    subst::SubstType,
    types::{OpLambdaSub, TypeGroup, TypeVariable},
};

impl<Lang> SolveConstraint for OpLambdaSub<Lang>
where
    Lang: Language,
    ...
    TypeVariable<Lang>: Into<Lang::Type>,
{
    ...
    fn solve_equality(
        ...
        let rhs_lam = rhs.into_oplambdasub().ok_or(err)?;
    ...
    fn solve_subtyping(
        ...
        let sup_lam = sup.into_oplambdasub().ok_or(err)?;
        ...
        state.add_constraint(SubtypeConstraint::new(
            Rc::unwrap_or_clone(self.sup.clone()),
            Rc::unwrap_or_clone(sup_lam.sup),
        ));
        state.add_tyvar(&self.var, Rc::unwrap_or_clone(self.sup));
        Ok(())
    }
}
