...
use syntax::{
    ...
    span::Spanned,
    subst::SubstType,
    types::{Forall, TypeGroup, TypeVariable},
};

impl<Lang> SolveConstraint for Forall<Lang>
where
    Lang: Language,
    ...
    TypeVariable<Lang>: Into<Lang::Type>,
{
    ...
    fn solve_equality(
        ...
        let err = TypeMismatch::new(rhs.to_string(), "Universal Type".to_string(), rhs.span());
        let rhs_forall = rhs.into_forall().ok_or(err)?;
    ...
    fn solve_subtyping(
        ...
        let err = TypeMismatch::new(sup.to_string(), "Universal Type".to_string(), sup.span());
        let sup_forall = sup.into_forall().ok_or(err)?;
