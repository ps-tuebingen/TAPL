...
impl TestSuite for TypedArithTests {
    ...
    fn load(&self) -> Result<Vec<Box<dyn Test>>, Error> {
        ...
        for content in contents {
            let parse_test = ParseTest::<typed_arithmetic::syntax::Term>::new(
                &content.source_name,
                &content.source_contents,
            );
            tests.push(Box::new(parse_test) as Box<dyn Test>);
            let reparse_test = ReparseTest::<typed_arithmetic::syntax::Term>::new(
                &content.source_name,
                &content.source_contents,
            );
            ...
            let check_test = CheckTest::<typed_arithmetic::syntax::Term>::new(
                &content.source_name,
                &content.source_contents,
                &content.conf.ty,
            );
