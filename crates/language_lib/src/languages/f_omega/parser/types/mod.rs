...
use syntax::types::{Bool, Fun, Nat, OpApp, TypeVariable, Unit};
...
fn pair_to_primtype(p: Pair<'_, Rule>) -> Result<Type, Error> {
    match p.as_rule() {
        ...
        Rule::forall_kinded_type => pair_to_universal(p).map(|uni| uni.into()),
        Rule::forall_unbounded_type => pair_to_universal_star(p).map(|uni| uni.into()),
        Rule::op_lambda_star_type => pair_to_op_lambda_star(p).map(|oplam| oplam.into()),
        Rule::op_lambda_type => pair_to_op_lambda(p).map(|oplam| oplam.into()),
        Rule::exists_kinded_type => pair_to_exists(p).map(|ex| ex.into()),
