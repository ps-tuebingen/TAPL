...
fn pair_to_leftrec_type(p: Pair<'_, Rule>, ty: Type) -> Result<Type, Error> {
    match p.as_rule() {
        ...
        Rule::pair_type => pair_to_pair_type(p, ty),
...
fn pair_to_pair_type(p: Pair<'_, Rule>, ty: Type) -> Result<Type, Error> {
    let inner = pair_to_n_inner(p, vec!["Pair Second Type"])?.remove(0);
    let snd_ty = pair_to_type(inner)?;
    Ok(Type::Pair(Box::new(ty), Box::new(snd_ty)))
}
